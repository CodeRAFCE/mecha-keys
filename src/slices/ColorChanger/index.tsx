...
const ColorChanger: FC<ColorChangerProps> = ({ slice }) => {
  ...
  return (
    <section
      ...
      className="min-h=[1000px] relative flex h-[90vh] flex-col overflow-hidden bg-linear-to-br from-[#0f172a] to-[#062f4a] text-white"
    >
      {/* SVG Background */}
      <svg
        ...
        >
          {Array.from({ length: 20 }, (_, index) => (
            <tspan
              ...
              dy={index === 0 ? -50 : 6}
            >
              {Array.from({ length: 10 }, (_, i) => backgroundText).join("")}
            </tspan>
          ))}
        </text>
      </svg>
      ...
      {/* Canvas */}
      <Canvas
        camera={{ position: [0, 0.5, 0.4], fov: 45, zoom: 1.5 }}
